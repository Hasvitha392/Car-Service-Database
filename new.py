...
def searchspares():
    try:
            new={}
            new[0]=input("Give spare part name to search: ")
            query = " SELECT * FROM Spares WHERE name='%s';" % (new[0])
            cur.execute(query)
            rows = cur.fetchall()
            for row in rows:
                print(row)
...
def searchcar():
    try:
            new={}
            new[0]=input("Give reg_id of car to search: ")
            query1=" SELECT * FROM Cars WHERE reg_id='%s';" % (new[0])
            cur.execute(query1)
            rows1 = cur.fetchall()
            for row1 in rows1 :
                print(row1)
